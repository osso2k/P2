      ...
      {tasks.length > 0 ? (
        tasks.map((task) => (
            <div key={task.id} className="flex-col max-w-[80%] border rounded-lg mb-1">
                <h2 className="text-left text-sm font-bold font-serif pb-2 px-3 mt-1 wrap-break-word">{task.title}</h2>  
                <div className="flex flex-wrap my-1 mx-2">
                  <p className="text-xs font-medium font-sans mx-auto">{task.time.split(":")[0]}:{task.time.split(":")[1]}</p>
